...
public class P08LettersChangeNumbers {
    public static void main(String[] args) {
        ...
        for (String element : numbers) {
            char before = element.charAt(0);
            double num = Double.parseDouble(element.substring(1, element.length() - 1));
            char after = element.charAt(element.length() - 1);
            double currentResult = 0.00;
            int position = 0;
            if (before >= 65 && before <= 90) {
                position = before - 64;
                currentResult = (num / position);
...
Depending on whether the letter was in front of the number
...
A is Uppercase and its position in the alphabet is 1. So we divide the number 12 with position 1 (12/1 = 12). Then we move to the letter after the number. b is lowercase
and its position is 2. So we add 2 to the resulting number (12+2=14).
