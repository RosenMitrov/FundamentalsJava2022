...
//created by R.M.
public class Fund05DrumSet {
    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);

        double savings = Double.parseDouble(scanner.nextLine());
        List<Integer> initialDrumSet = Arrays.stream(scanner.nextLine().split(" ")).map(Integer::parseInt).collect(Collectors.toList());
        ...
        while (!input.equals("Hit it again, Gabsy!")) {
            ...
            for (int i = 0; i < drumSet.size(); i++) {
                ...
                if (drumSet.get(i) <= 0) {
                    ...
                    double price = initialDrumSet.get(i) * 3;
                    if (savings - price < 0) {
                        drumSet.remove(i);
...
Until you receive the command "Hit it again, Gabsy!", you will be receiving integer: the hit power Gabsy applies on each drum while practicing.
...
Gabsy will always replace her broken drums until the moment she can no longer afford them. If she doesn't have enough money for a replacement,
